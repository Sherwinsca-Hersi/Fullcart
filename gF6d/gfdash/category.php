    ...
    <div class="banner_rightbar container">
        ...
        <div class="searchbar_sect">
            <div class="search_div">
                <input type="text" name="search_input" placeholder="Search Category Name.." id="customSearchBox" class="searchInput">  
            </div>
            <div class="addBanner_sect">
                <a href="addcategory.php" class="export_btn employee_link"> Add Category </a>
...
editIcon.addEventListener('click', function(){
    ...
    const url = `addcategory.php?categoryid=${checkedRowIds.join(',')}`;
...
deleteIcon.addEventListener('click', function(){
    const { checkedRowIds, selectedBatchValues } = displayCheck();
    if (checkedRowIds.length > 0) {
    const url = `com_ins_upd.php?cat_dids=${checkedRowIds.join(',')}`;
    window.location.href = url;
    } else {
...
<script>
        ...
        const fetchedData = [
            <?php
            $export_query=$mysqli->query("SELECT title,c_img,active from `e_category_details` where cos_id = '$cos_id' and active=1");

     
            $category_details= [];
            while ($category_export = $export_query->fetch_assoc()){
                $category_details[] = $category_export;
            }
        ...
        function exportData() {
        const headers = ["Category Title", "Category Image", "Active"];
        ...
        headers.forEach((header, index) => {
            ...
            let fileName = "category_data.xlsx";
            saveAs(new Blob([s2ab(wbout)], { type: "application/octet-stream" }), fileName);
        }
    </script>
<script>
function redirect(row, categoryId) {
    ...
    if (!checkbox.checked) {
        window.location.href = 'addcategory.php?categoryid=' + categoryId;
    }
}
</script>
