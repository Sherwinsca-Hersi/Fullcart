...
<?php 
    ...
    <div class="report_rightbar container">
        ...
        <table rules='all' cellpadding='50px' cellspacing='50px' class="table_style display" id="example" data-disablesortingcolumns="4,9,10">
                    ...
                    <?php
                        $i=1;
                        foreach($lowStockAlert as $lowStockProduct):
                        ?>
                        ...
                        <td><?php echo $i; ?></td>
                        <td><?php echo $lowStockProduct['sku_id'] ?></td>
                        <td  class="highlight"><?php echo $lowStockProduct['batch_no']; ?></td>
                        <td  class="highlight"><a href="allbatches.php?product_id='<?php echo $lowStockProduct['product_id'];?>'"><?php echo $lowStockProduct['p_title'].' '.$lowStockProduct['p_variation'] .''. $lowStockProduct['unit']; ?></a></td>
                        <td><?php echo $lowStockProduct['mrp']; ?></td>
                        <td><?php $p_stock_query = $mysqli->query("select in_price from e_product_stock where cos_id = '$cos_id' and s_product_id=".$lowStockProduct['product_id']." AND s_batch_no = '" . $lowStockProduct['batch_no'] . "'")->fetch_assoc();
                            echo $p_stock_query['in_price']; 
                        ?></td>
                        <td><?php echo $lowStockProduct['out_price']; ?></td>
                        <td  class="highlight"><?php echo $lowStockProduct['qty_left']??'N/A'; ?></td>
                        <td><?php echo $lowStockProduct['reorder_level']??'N/A'; ?></td>
                        <td  class="highlight"><?php echo $lowStockProduct['emergency_level']??'N/A'; ?></td>
                        <td data-sort='<?php echo date_format(date_create($lowStockProduct['created_ts']), "Ymd"); ?>'>
                            <?php 
                                $date = date_create($lowStockProduct['created_ts']);
                                echo date_format($date, "d/m/Y h:i A");
                            ?>
                        </td>
                        <td data-sort='<?php echo date_format(date_create($lowStockProduct['updated_ts']), "Ymd"); ?>'>
                            <?php 
                                $date = date_create($lowStockProduct['updated_ts']);
                                echo date_format($date, "d/m/Y h:i A");
                            ?>
                        </td>
                    </tr>
...
<script>
        ...
        const fetchedData = [
            <?php
            $export_query=$mysqli->query("SELECT pd.sku_id,pp.id,pd.p_title,pd.p_variation,p.batch_no,p.mrp,p.out_price,p.qty_left,p.expiry_date,pd.reorder_level,pd.emergency_level
FROM e_product_price p
JOIN e_product_details pd ON p.product_id = pd.id and p.cos_id = '$cos_id' and pd.cos_id = '$cos_id'
WHERE p.qty_left<=pd.emergency_level and p.qty_left > 0");
     
            $lowStockAlert_details= [];
            while ($lowStockAlert_export = $export_query->fetch_assoc()){
                $lowStockAlert_details[] = $lowStockAlert_export;
            }
            foreach ($lowStockAlert_details as $row) {
        ...
        function exportData() {
        const headers = ["Model No", "Product Id","Product Title", "Variation", "Batch No", "MRP", "Outprice", "Current Stock", "Expiry Date","Reorder Level", "Emergency Level"];
        ...
        headers.forEach((header, index) => {
            ...
            let fileName = "lowstock_data.xlsx";
