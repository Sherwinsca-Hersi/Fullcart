...
while($row = $coll->fetch_assoc())
{
	...
	$plist = $mysqli->query("SELECT *
        ...
        WHERE pd.active=1 AND pd.is_loose=0 and sub_cat_id=".$row['id']." and pd.cos_id = '$cos_id' GROUP BY pd.id");
	$products = array();
	$lp = array();
	...
	$catname = $mysqli->query("select * from e_category_details where id=".$rows['cat_id']." and cos_id = '$cos_id'")->fetch_assoc();
