...
if($user_id == '') {
    ...
    $counter = $mysqli->query("SELECT f.user_id, f.product_id, f.added_on, f.updated_on, f.active,pd.id,pd.city_id,pd.unit,
       ...
       pd.igst,pd.p_disc,pd.p_disc,pd.type,pd.p_desc, ps.s_disc, pp.mrp, pp.out_price, pp.qty_left, pp.batch_no
        ...
        LEFT JOIN e_product_details pd ON pd.id = f.product_id AND pd.active = 1 AND pd.is_loose=0 AND pd.cos_id = '$cos_id'
