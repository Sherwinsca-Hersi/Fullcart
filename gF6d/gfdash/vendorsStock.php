    ...
    <div class="report_rightbar container">
        ...
        <table rules='all' cellpadding='50px' cellspacing='50px' class="table_style display" id="example" data-disablesortingcolumns="0,6,7">
                    ...
                    if (isset($_GET['supplierid'])){
                        $vendor_stock=$mysqli->query("SELECT * FROM e_product_details pd,e_product_stock ps,e_product_price pp
                        ...
                        AND pd.cos_id=$cos_id AND ps.active=1 AND ps.cos_id=$cos_id AND ps.supplier_id=".$_GET['supplierid']." ORDER BY ps.created_ts DESC");
                    }
                    else if(isset($_GET['invoice_no'])){
                        $vendor_stock=$mysqli->query("SELECT * FROM e_product_details pd,e_product_stock ps,e_product_price pp
                        WHERE pd.id = ps.s_product_id AND ps.s_product_id = pp.product_id AND pp.product_id=pd.id AND pp.batch_no=ps.s_batch_no AND pp.qty_left > 0 AND pp.cos_id=$cos_id AND pd.active=1
                        AND pd.cos_id=$cos_id AND ps.active=1 AND ps.cos_id=$cos_id AND ps.invoice_no=".$_GET['invoice_no']." ORDER BY ps.created_ts DESC");
                    }
                    else{
                        $vendor_stock=$mysqli->query("SELECT * FROM e_product_details pd,e_product_stock ps,e_product_price pp
                        WHERE pd.id = ps.s_product_id AND ps.s_product_id = pp.product_id AND pp.product_id=pd.id AND pp.batch_no=ps.s_batch_no AND pp.qty_left > 0 AND pp.cos_id=$cos_id AND pd.active=1
                        AND pd.cos_id=$cos_id AND ps.active=1 AND ps.cos_id=$cos_id ORDER BY ps.created_ts DESC");
                    }
...
<script>
        ...
        const fetchedData = [
            <?php
            $export_query=$mysqli->query("SELECT invoice_no,s_prduct_id,s_batch_no,p_title,p_variation,unit,qty,created_ts,supplier_id,stock_bill FROM e_product_details pd,e_product_stock ps,e_product_price pp
                        WHERE pd.id = ps.s_product_id AND ps.s_product_id = pp.product_id AND pp.product_id=pd.id AND pp.batch_no=ps.s_batch_no AND pp.qty_left > 0 AND pp.cos_id=$cos_id AND pd.active=1
                        AND pd.cos_id=$cos_id AND ps.active=1 AND ps.cos_id=$cos_id ORDER BY ps.created_ts DESC");
        ...
        function exportData() {
        const headers = ["Invoice No", "Product Id", "Batch No", "Product Title","Product Variation","Total Stock","Created Date/Time","Supplier Id","Stock Bill"];
