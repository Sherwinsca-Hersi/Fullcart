...
$reorder_level_query=$mysqli->query("SELECT 
    pd.sku_id, p.batch_no, p.product_id, 
    pd.p_title, pd.p_variation, pd.unit, 
    p.mrp, p.out_price, p.qty_left, 
    pd.reorder_level, pd.emergency_level, 
    p.updated_ts, p.created_ts
FROM 
    e_product_price p
JOIN 
    ...
    AND p.cos_id = '$cos_id'
    AND pd.cos_id = p.cos_id
WHERE 
    p.active = 1
    AND pd.active = 1
    AND p.qty_left <= pd.reorder_level
    AND p.qty_left > pd.emergency_level
ORDER BY 
    p.p_id DESC");
...
$missing_stock_query=$mysqli->query("SELECT 
    pd.sku_id, p.batch_no, p.product_id, 
    pd.p_title, pd.p_variation, pd.unit, 
    p.mrp, p.out_price, p.qty_left, 
    pd.reorder_level, pd.emergency_level, 
    p.updated_ts, p.created_ts, p.missing_qty
FROM 
    e_product_price p
JOIN 
    e_product_details pd 
...
// purchase Entry
$purchase_entry_query=$mysqli->query("SELECT  `id`, `cos_id`, `invoice_no`, `supplier_id`,
 `stock_bill`,`created_ts`, `updated_ts`,`active`  FROM `e_purchase_entry` 
...
$cust_query=$mysqli->query("SELECT  `id`, `name`, `mobile`, `email_id`, `password`, `whatsapp`,`wallet`
 FROM `e_user_details`  WHERE active=1 AND  cos_id = '$cos_id' ORDER BY id DESC");
...
while ($unit_table = $unit_query->fetch_assoc()){
    $units[] = $unit_table;
}
...
// $real_query = $mysqli->query("SELECT id,invoice_no,created_ts,name,o_total,created_by, DATE_FORMAT(updated_ts, '%d %m %Y') AS formatted_date  FROM `e_normal_order_details` where DATE(updated_ts) BETWEEN  '$startDate1' AND '$endDate1' AND cos_id = '$cos_id' and active='6' and bill_type='2' order by id desc");
...
// //completed
// $completed_query = $mysqli->query("SELECT id,created_ts,name,o_total,p_method_id,trans_id,upi_id,t_slot,updated_by,salesman_id,status,updated_ts , DATE_FORMAT(updated_ts, '%d %m %Y') AS formatted_date FROM `e_normal_order_details` 
// where DATE(updated_ts) BETWEEN '$startDate1' AND '$endDate1' AND cos_id = '$cos_id' and active='6'  and bill_type='1' order by id desc"); 
// //processing
// $processing_query = $mysqli->query("SELECT id,created_ts,name,o_total,p_method_id,trans_id,upi_id,t_slot,updated_by,salesman_id,status,updated_ts , DATE_FORMAT(updated_ts, '%d %m %Y') AS formatted_date FROM `e_normal_order_details` 
// where DATE(updated_ts) BETWEEN '$startDate1' AND '$endDate1' AND cos_id = '$cos_id' and active='2'  and bill_type='1' order by id desc");
// //packed
//     $packed_query = $mysqli->query("SELECT id,created_ts,name,o_total,p_method_id,trans_id,upi_id,t_slot,updated_by,salesman_id,status,updated_ts , DATE_FORMAT(updated_ts, '%d %m %Y') AS formatted_date FROM `e_normal_order_details` 
// where DATE(updated_ts) BETWEEN '$startDate1' AND '$endDate1' AND cos_id = '$cos_id' and active='3'  and bill_type='1' order by id desc");
// //db_assigned
// $db_assigned_query = $mysqli->query("SELECT id,created_ts,name,o_total,p_method_id,trans_id,upi_id,t_slot,updated_by,salesman_id,status,updated_ts , DATE_FORMAT(updated_ts, '%d %m %Y') AS formatted_date FROM `e_normal_order_details` 
// where DATE(updated_ts) BETWEEN '$startDate1' AND '$endDate1' AND cos_id = '$cos_id' and active='4'  and bill_type='1' order by id desc");
// //out_delivery
// $out_delivery_query = $mysqli->query("SELECT id,created_ts,name,o_total,p_method_id,trans_id,upi_id,t_slot,updated_by,salesman_id,status,updated_ts , DATE_FORMAT(updated_ts, '%d %m %Y') AS formatted_date FROM `e_normal_order_details` 
// where DATE(updated_ts) BETWEEN '$startDate1' AND '$endDate1' AND cos_id = '$cos_id' and active='5'  and bill_type='1' order by id desc");
// //pending
// $pending_query = $mysqli->query("SELECT id,created_ts,name,o_total,p_method_id,trans_id,upi_id,t_slot,updated_by,salesman_id,status,updated_ts , DATE_FORMAT(updated_ts, '%d %m %Y') AS formatted_date FROM `e_normal_order_details` 
