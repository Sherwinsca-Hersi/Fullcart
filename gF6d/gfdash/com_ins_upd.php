...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    elseif (isset($_POST["product_add"])) {
        ...
        if (isset($_FILES['main_prod_img']) && !empty($_FILES['main_prod_img']['name'][0])) {
            if (empty($_FILES["main_prod_img"]["name"])) {
                ...
                if (isset($_FILES['prod_img']) && !empty($_FILES['prod_img']['name'][0])) {
                    $uploadedFiles = $_FILES['prod_img'];
                    ...
                    for ($i = 0; $i < $fileCount; $i++) {
                        ...
                        $targetFile = $uploadDir . basename($safeFileName);
                        ...
                        if ($fileError === 0) {
                                ...
                                if (in_array($fileType, $allowedTypes)) {
                                    if (!file_exists($targetFile)) {
                                        ...
                                        if (move_uploaded_file($fileTmpName, $newTargetFile)) {
                                            $uploadedImages[] = $newTargetFile;
                                        }
                                    }
                                }
                                else{
                ...
                $imagePathsString = !empty($uploadedImages) ? implode("|", array_map(function ($filePath) {
                    return str_replace('../', '', $filePath);
                }, $uploadedImages)) : ''; // Optional handling: empty string if no images uploaded
    ...
    elseif (isset($_POST["product_update"])) {
        ...
        $currentImages = $prod_query['imgs'];
    
        $deletedImages = isset($_POST['deleted_images']) ? explode('|', $_POST['deleted_images']) : [];
    
        $sanitizedDeletedImages = array_map(function($image) {
            return strpos($image, '../') === 0 ? substr($image, 3) : $image;
        }, $deletedImages);
    
        $uploadedImages = [];
        $imagePathsString = $currentImages;
        ...
        if (isset($_FILES['main_prod_img']) && !empty($_FILES['main_prod_img']['name'])) {
            ...
            $fileMimeType = mime_content_type($_FILES["main_prod_img"]["tmp_name"]);
            if (in_array($fileMimeType, $disallowedTypes)) {
                $_SESSION['error_message'] = "Main product image type '$fileMimeType' is not allowed.<br>";
        ...
        if (isset($_FILES['prod_img']) && !empty($_FILES['prod_img']['name'][0])) {
            ...
            for ($i = 0; $i < $fileCount; $i++) {
                ...
                if (in_array($fileMimeType, $disallowedTypes)) {
                    $_SESSION['error_message'] = "File type '$fileMimeType' is not allowed for file '$fileName'.<br>";
                    header("Location: addproduct.php?productid=$product_id");
                    exit();
                }
        ...
        if ($currentImages) {
            ...
            $finalCurrentImages = array_diff($currentImagesArray, $sanitizedDeletedImages);
        } else {
            $finalCurrentImages = [];
        }
    
        $finalUploadedImages = array_merge($finalCurrentImages, $uploadedImages);
        $imagePathsString = !empty($finalUploadedImages) ? implode("|", $finalUploadedImages) : '';
        ...
        try {
            $update_query1 = "UPDATE e_product_details 
                              SET sku_id = '$sku_id', 
                                  ...
                                  p_img = '$currentMainImage', 
                                  imgs = '$imagePathsString', 
                                  p_title = '$p_title', 
            ...
            $result = $mysqli->query($update_query1);
            if ($result) {
    ...
    //Stock Add
            ...
            if($batchno_query['total'] > 0){
                ...
                try{
                    ...
                    if(strpos($targetFile, ".png") !== false || strpos($targetFile, ".jpg") !== false || strpos($targetFile, ".jpeg") !== false || strpos($targetFile, ".svg") !== false || strpos($targetFile, ".jfif") !== false || strpos($targetFile, ".webp") !== false){
                        $fields2="`cos_id`,`s_product_id`,`qty`,`stock_bill`,`purchase_id`,`invoice_no`,`supplier_id`,`s_batch_no`,`in_price`,`s_mrp`,`s_out_price`,`platform`";
                        $values2="'$cos_id', '$product_id', '$tstock','$targetFile','$purchase_id','$invoice_no','$supplier_id', '$batch_no','$in_price', '$out_price', '$out_price','$platform'";              
                        ...
                        $fields2="`cos_id`,`s_product_id`,`qty`,`purchase_id`,`invoice_no`,`supplier_id`,`s_batch_no`,`in_price`,`s_mrp`,`s_out_price`,`platform`";
                        $values2="'$cos_id', '$product_id', '$tstock','$purchase_id','$invoice_no','$supplier_id', '$batch_no','$in_price', '$out_price', '$out_price','$platform'";          
                        $stock_insert="INSERT INTO  `e_product_stock` ($fields2) VALUES ($values2)";
                        $insert_query2=$mysqli->query($stock_insert);
                        if (!$insert_query2) {
                            throw new Exception("Error occurred during the insert into e_product_stock: " . mysqli_error($mysqli));
                    ...
                    $fields3="`cos_id`,`product_id`,`mrp`,`out_price`,`batch_no`,`qty_left`,`platform`";
                    $values3="'$cos_id','$product_id', '$out_price', '$out_price', '$batch_no', '$tstock','$platform'";              
            ...
            if(empty($_FILES["stock_bill"]["name"])){
                ...
                //     } else {
                        try{
                            ...
                            `in_price`='$in_price',`s_mrp`='$out_price',`s_out_price`='$out_price',`invoice_no`='$invoice_no',
                            ...
                            $price_update="UPDATE `e_product_price` SET `batch_no`='$batch_no',`qty_left`='$tstock',`mrp`='$out_price',`out_price`='$out_price',
            ...
            if(strpos($targetFile, ".png") !== false || strpos($targetFile, ".jpg") !== false || strpos($targetFile, ".jpeg") !== false || strpos($targetFile, ".svg") !== false || strpos($targetFile, ".jfif") !== false || strpos($targetFile, ".webp") !== false){
                ...
                // } else {
                    try{
                        ...
                        $price_update="UPDATE `e_product_price` SET `batch_no`='$batch_no',`mrp`='$out_price',`out_price`='$out_price',`up_platform`='$platform'  WHERE `cos_id`= '$cos_id' AND `product_id`='$product_id' AND `batch_no`='$batch_id'";
                        // echo $price_update;
                        $update_query2=$mysqli->query($price_update);
                        // echo $update_query2;
                        $_SESSION['success'] = "Stock Details Updated Successfully!";
        ...
        if(isset($_FILES["stock_bill"])) {
            ...
            try {
                ...
                foreach($productId as $index => $pid) {
                    ...
                    $fields2 = "`cos_id`, `s_product_id`, `qty`, `stock_bill`, purchase_id,`invoice_no`, `supplier_id`, `s_batch_no`,`in_price`, `s_mrp`, `s_out_price`,`platform`";
                    $values2 = "'$cos_id','$pid','$current_tstock','$targetFile','$purchase_id','$invoiceNo', '$supplierId', '$current_batchNo','$current_in_price', '$current_out_price', '$current_out_price','$platform'";
                    $stock_insert = "INSERT INTO `e_product_stock` ($fields2) VALUES ($values2)";
                    // echo $stock_insert."<br>";
                    $insert_query2 = $mysqli->query($stock_insert);
                    if (!$insert_query2) {
                        throw new Exception("Error occurred during the insert into e_product_stock: " . mysqli_error($mysqli));
                    }
                    ...
                    $fields3 = "`cos_id`, `product_id`, `mrp`, `out_price`, `batch_no`, `qty_left`,`platform`";
                    $values3 = "'$cos_id', '$pid', '$current_out_price', '$current_out_price', '$current_batchNo', '$current_tstock','$platform'";
                    $price_insert = "INSERT INTO `e_product_price` ($fields3) VALUES ($values3)";
                    // echo $price_insert."<br>";
                    $insert_query3 = $mysqli->query($price_insert);
                    if (!$insert_query3) {
                        throw new Exception("Error occurred during the insert into e_product_price: " . mysqli_error($mysqli));
                    }
            ...
            if(isset($_FILES["logo_img"])){
                ...
                if(empty($_FILES["logo_img"]["name"])){
                    ...
                    if(strpos($targetFile, ".png") !== false || strpos($targetFile, ".jpg") !== false || strpos($targetFile, ".jpeg") !== false || strpos($targetFile, ".svg") !== false || strpos($targetFile, ".jfif") !== false || strpos($targetFile, ".webp") !== false){
                        ...
                        $_SESSION['error_message'] = 'Image Format is Wrong';
                        $_SESSION['old_profile'] = $_POST;
                        header("Location: addProfile.php?profileid=$profile_id");
                        exit();
                    }
                ...
                $_SESSION['error_message'] = 'Please Upload the Profile Image';
                $_SESSION['old_profile'] = $_POST;
                header("Location: addProfile.php?profileid=$profile_id");
                exit();
            }
