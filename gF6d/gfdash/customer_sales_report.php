    ...
    <div class="customer_sales_rightbar container">
        ...
        <table rules='all' cellpadding='50px' cellspacing='50px' class="table_style display" id="example" data-disablesortingcolumns="0,4">
                    ...
                    if(isset($_GET['customer_id'])){
                        $cust_sales_query=$mysqli->query("SELECT * FROM `e_normal_order_details` WHERE cos_id='$cos_id' and active=1 and u_id=".$_GET['customer_id']." ORDER BY created_ts DESC");
                    }
                    else{
                        $cust_sales_query=$mysqli->query("SELECT * FROM `e_normal_order_details` WHERE cos_id='$cos_id' and active=1 ORDER BY created_ts DESC");
                    }
