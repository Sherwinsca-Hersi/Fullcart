...
try{
    ...
    for ($i = 0; $i < count($ProductData); $i++) {
        ...
        $field_values = array("cos_id", "o_id", "batch_no", "p_quantity", "p_title", "p_discount", "p_img", "p_price", "p_type", "platform");
        $data_values = array("$cos_id", "$o_id", "$batch_no", "$qty", "$title", "$discount", "$image", "$cost", "$type", "$platform");
        $h = new CommonFunction();
