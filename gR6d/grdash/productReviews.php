    ...
    <div class="banner_rightbar container">
        ...
        <table rules='all' cellpadding='50px' cellspacing='50px' class="table_style display" id="example" data-disablesortingcolumns="0,2">
                    ...
                    <tr class="<?php echo ($i % 2 === 0)? 'teven' : 'todd';?>">
                        <td><?php echo $i; ?></td>
                        <td><a href="singleProductReview.php?review_prod_id=<?php echo $review_detail['product_id'];?>"><?php 
                            $product_query=$mysqli->query("SELECT id,p_title FROM `e_product_details` WHERE id=".$review_detail['product_id']." AND active=1 AND cos_id='$cos_id'")->fetch_assoc();
                            echo $product_query['p_title'] ?? 'N/A'; 
                        ?></a></td>
                        <td>
                        <?php 
                            $rating_count=$mysqli->query("SELECT `id`
                            FROM `e_products_rating` WHERE product_id=".$review_detail['product_id']." AND active=1 AND cos_id='$cos_id'
                            ORDER BY id DESC")->num_rows;
                            if($rating_count > 1 ) {echo $rating_count .' Ratings';} else{echo $rating_count .' Rating';}; 
                        ?>
                        </td>
                    </tr>
                    <?php
