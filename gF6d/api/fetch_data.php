...
$out_stock_query=$mysqli->query("SELECT pd.id, pd.p_title, pd.p_variation, pd.unit, 
pd.sku_id,pd.reorder_level,pd.emergency_level,ps.s_product_id, ps.s_batch_no, ps.s_mrp, 
...
(ps.s_product_id IS NULL OR ps.active != 1) AND  pd.cos_id = '$cos_id' ORDER BY ps.s_id DESC");
$outOfStock=[];
