...
<?php 
    ...
    <div class="employee_rightbar container">
        ...
        <table rules='all' cellpadding='50px' cellspacing='50px' class="table_style display" id="example" data-disablesortingcolumns="4,5">
                    ...
                    ?>
                    <tr class="<?php echo ($i % 2 === 0)? 'teven' : 'todd';?>" onclick="redirect(this, <?php echo $customer_detail['user_id'];?>)">
                        ...
                        <td><a href="customer_sales_report.php?customer_id='<?php echo $customer_detail['user_id'];?>'"><?php  echo $customer_detail['name'];?></a></td>
                        ...
                        ?></td>
                        <td><?php  
                        if(($customer_detail['address_line_1'] && $customer_detail['landmark'] && $customer_detail['area'] && $customer_detail['city'] && $customer_detail['state'] && $customer_detail['country'] && $customer_detail['pincode']) == '' || NULL){
                            echo 'N/A';
                        }
                        else{
                            $cust_address=$customer_detail['address_line_1'].' ,'.$customer_detail['landmark'].' ,'.$customer_detail['area'].' ,'.$customer_detail['city'].' ,'.$customer_detail['state'].' ,'.$customer_detail['country'].' -'.$customer_detail['pincode'];
                            echo $cust_address;
                        }?></td>
                        <?php 
                            $cust_order_query=$mysqli->query("SELECT created_ts,subtotal FROM `e_normal_order_details` WHERE cos_id='$cos_id' and active=1 and u_id=".$customer_detail['user_id']." ORDER BY created_ts DESC LIMIT 1")->fetch_assoc();
                        ?>
