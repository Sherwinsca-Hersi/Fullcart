...
<head>
    ...
    <title>Multiple Stock Details</title>
    <?php 
...
<?php 
    ...
    <div class="product_rightbar container" id="divprint">
        <h2>Multiple Stock Details</h2>
        <div class="searchbar_sect">
            <div class="search_div">
                <input type="text" name="search_input" placeholder="Search Product Name,invoice,Batch No.." id="customSearchBox" class="searchInput">  
            </div>
        ...
        <table rules='all' cellpadding='50px' cellspacing='50px' class="table_style display" id="example" data-disablesortingcolumns="0,2,3">
                    ...
                    <tbody>
                    <?php
                    $product_stock=$mysqli->query("SELECT * FROM `e_product_stock` WHERE purchase_id=".$_GET['purchaseid']."");
                    $multiple_stock=[];
                    while($multiple_stock_details=$product_stock->fetch_assoc()){
                        $multiple_stock[]=$multiple_stock_details;
                    }
                    $i=1;
                        foreach($multiple_stock as $multiple_stock):
                        ?>
                        <tr class="<?php echo ($i % 2 === 0)? 'teven' : 'todd';?>"  onclick="redirect(this, '<?php echo $multiple_stock['s_product_id'];?>','<?php echo $multiple_stock['s_batch_no'];?>')">
                        <!-- <td><input type="checkbox" class="check_value checkbox_tdef" name="checkbox" data-id="<?php echo $multiple_stock['s_product_id']?>" data-batch="<?php echo $multiple_stock['s_batch_no'];?>">&emsp;<?php echo $i; ?></td> -->
                        <td><?php echo $i; ?></td>
                        <td><?php $stock_skuid=$mysqli->query("select sku_id from e_product_details where cos_id='$cos_id' AND id=".$multiple_stock['s_product_id']."")->fetch_assoc();
                            echo $stock_skuid['sku_id'];
                        ?>
                        </td>
                        <td class="highlight"><?php echo $multiple_stock['s_batch_no']; ?></td>
                        <td>
                            <?php $product_name=$mysqli->query("select p_title,p_variation,unit from e_product_details where cos_id='$cos_id' AND id=".$multiple_stock['s_product_id']."")->fetch_assoc();
                                echo $product_name['p_title']." ".$product_name['p_variation']." ".$product_name['unit'];
                            ?>
                        </td>
                        <td><?php echo $multiple_stock['s_mrp']; ?></td>
                        <td><?php echo $multiple_stock['in_price']; ?></td>
                        <td><?php echo $multiple_stock['s_out_price']; ?></td>
                        <td data-sort='<?php echo date_format(date_create($multiple_stock['s_expiry_date']), "Ymd"); ?>'>
                            <?php 
                                $date = date_create($multiple_stock['s_expiry_date']);
                                echo date_format($date, "d/m/Y");
                                    ?>
                        </td>
                        <td class="highlight"><?php echo $multiple_stock['qty']; ?></td>
                        <!-- <td>
                    <?php  
                    $vendors_name = $mysqli->query("select v_name from e_vendor_details where cos_id = '$cos_id' and v_id=".$multiple_stock['supplier_id']."")->fetch_assoc();
                    echo $vendors_name['v_name'] ?? 'N/A';
                    ?>
                </td>
                <td>
                    <?php  
                    $vendors_phone = $mysqli->query("select v_mobile from e_vendor_details where cos_id = '$cos_id' and v_id=".$multiple_stock['supplier_id']."")->fetch_assoc();
                    echo $vendors_phone['v_mobile'] ?? 'N/A';
                    ?>
                </td> -->
                <td data-sort='<?php echo date_format(date_create($multiple_stock['updated_ts']), "Ymd"); ?>'>
                            <?php 
                                $date = date_create($multiple_stock['updated_ts']);
...
<script>
function redirect(row, productId,batchId) {
    window.location.href = `inventory_addStock.php?productid=` + productId +`&&batch_id=` + batchId;
