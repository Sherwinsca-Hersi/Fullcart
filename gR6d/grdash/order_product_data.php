...
<div>
    ...
    <?php
        ...
        $c = $mysqli->query("select `id`, `cos_id`, `u_id`, `invoice_no`, `o_date`, `p_method_id`,
         `address`, `landmark`, `d_charge`, `cou_id`, `cou_amt`, `o_total`, `subtotal`, `tot_sgst`,
          `tot_cgst`, `tot_igst`, `trans_id`, `payment_active`, `salesman_id`, 
        `wall_amt`, `name`, `mobile`, `status`, `bill_type`, `bank_trans_id`,
         `upi_id`, `recon_status`,`active` from e_normal_order_details where cos_id = '$cos_id' and id=".$p_id."")->fetch_assoc();   

        $udata = $mysqli->query("select `id`, `cos_id`, `name`,
         `mobile`, `email_id`, `password`, `whatsapp`,
         `r_date`, `c_code`, `code`, `refer_code`, `wallet`, `type`, 
         `active` from e_user_details where cos_id = '$cos_id' and id=".$c['u_id']."")->fetch_assoc();

        $pdata = $mysqli->query("select `id`, `cos_id`, `title`, `img`, `attributes`, `subtitle`, `p_show`,`active` from e_dat_payment where cos_id = '$cos_id' and id=".$c['p_method_id']."")->fetch_assoc();
    ?>
    <div id="divprint">
      ...
      <div class="preview_row2">
        ...
        <table rules='all' cellpadding='50px' cellspacing='50px' class="table_style display">
          ...
          <?php 
                      ...
                      $get_data = $mysqli->query("select `id`, `cos_id`, `o_id`, `invoice_no`, 
                      `batch_no`, `p_quantity`, `p_title`, `p_discount`, `p_img`, `p_price`, `p_type`,
                      `created_ts`, `updated_ts`, `active`  from e_normal_order_product_details where cos_id = '$cos_id'
                       and o_id=".$p_id."");
  ...
  async function downloadPDF() {
    try {
        ...
        if (!element) {
            ...
            alert('Element not found. Please check the selector.');
            return;
        }
