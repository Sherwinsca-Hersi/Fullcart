...
if($keyword == '') {
    ...
    $counter = $mysqli->query("SELECT pd.id as productid,  pd.sku_id, pd.hsn_code, pd.barcode, pd.cat_id, pd.sub_cat_id, pd.p_img, pd.p_title, pd.brand, pd.imgs,pd.is_loose,
                                ...
                                WHERE pd.active = 1 AND pd.cos_id = '$cos_id'
                                    AND (
